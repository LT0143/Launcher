...
public class UpdateHelper
{
    ...
    public string serverlauncherFolder = "Launcher/";  // 启动器的服务器下载路径 包括MD5和EXE
    public string serverZipFolder = "ClientZip/";// 完整客户端zip的路径 包括MD5 和zip
    //public string localZipmd5Path = "";// 本地压缩包的md5的
    // 检查完整性的下载地址
    public string completeFolder = "Client/";//检查完整性需要下载文件的地址 这个全是零散文件
    public string VersionFolder = "Version/";//检查完整性需要下载文件的地址 这个全是零散文件
...
#else
    public string m_serverIp = "sowdown.playgate8.com:30008";  // 自己ID IP sw2.dindanw.com:8314  192.168.4.244
#endif
    //    public string m_serverIp = "192.168.4.141";  // 自己FTP        FTP !!!!!!!!!!!! IP
    ...
    private void ThreadGetClientMD5(object _value)
    {
        ...
        if (clientPathFile != null)
        {
            ...
            if (FileMatchUp.MakeEncodingList(_ServerUpdateStructs,clientPathFile, m_clientmd5Txt) == true)
            {
                ...
                sRclient.Close();
                _m_MainForm.UpdateProgressBarOnce();
